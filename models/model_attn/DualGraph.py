...
def compute_offset_loss(pd_offset, gt_center_left, gt_center_right, scale=0.15, weight=10.0):
    gt_center_left = gt_center_left.cuda() if not gt_center_left.is_cuda else gt_center_left
    gt_center_right = gt_center_right.cuda() if not gt_center_right.is_cuda else gt_center_right
    gt_offset = (gt_center_right - gt_center_left) / scale
    if gt_offset.dim() == 3 and gt_offset.size(1) == 1:
        gt_offset = gt_offset.squeeze(1)
    loss = F.smooth_l1_loss(pd_offset, gt_offset) * weight

    return loss, gt_offset
